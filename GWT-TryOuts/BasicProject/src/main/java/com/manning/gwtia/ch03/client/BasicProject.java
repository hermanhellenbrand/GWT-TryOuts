...
public class BasicProject implements EntryPoint, ValueChangeHandler<String> {
	...
	/**
	 * Strings representing the history tokens we will use to indicate which tab
	...
	/**
	 * An enumeration covering the 3 pages that will be involved in the
	 * application and the history.
	 * 
	 * Each enumerated item contain 2 pieces of information: a number that
	 * relates to the deck card number, and a String that relates to the history
	 * token.
	 * 
	 ...
	 */
	enum Pages {
		HOME(DECK_HOME, TOKEN_HOME), PRODUCTS(DECK_PRODUCTS, TOKEN_PRODUCTS), CONTACT(
				DECK_CONTACT, TOKEN_CONTACT);
				...
				if (searchRequest == null) {
					...
					searchTerm.addChangeHandler(new ChangeHandler() {
						public void onChange(ChangeEvent event) {
							...
							searchRequest.hide();
							// "start" the search
							Window.alert("If implemented, now we would search for: "
									+ searchTerm.getText());
						}
					});
	...
	/**
	 * The HTMLPanels that will hold the content we want to display in the
	 * TabPanel.
	 */
	HTMLPanel homePanel;
	 ...
	 * 
	 * 1. A TabPanel that holds "page" content from the original HTML page 2. A
	 * search button that is from the original HTML page 3. An image for the
	 * logo 4. A feedback bar.
	 * 
	 */
